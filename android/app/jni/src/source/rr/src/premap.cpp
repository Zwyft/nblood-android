...
static void G_PrecacheSprites(void)
{
    ...
    for (i = MENUSCREEN; i < DUKECAR; ++i)
        tloadtile(i, 1);
    for (i = TEXTSTORY; i <= RRTILE1636; ++i)
        tloadtile(i, 1);
    tloadtile(MENUBAR, 1);
    tloadtile(INGAMEDUKETHREEDEE, 1);
    tloadtile(DUKENUKEM, 1);
    tloadtile(THREEDEE, 1);
    tloadtile(BOTTOMSTATUSBAR, 1);
    tloadtile(TEXTBOX, 1);
    tloadtile(WINDOWBORDER1, 1);
    tloadtile(WINDOWBORDER2, 1);
    tloadtile(CDPLAYER, 1);
    tloadtile(LOADSCREEN, 1);
    tloadtile(BONUSSCREEN, 1);
}

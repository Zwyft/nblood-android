...
static FORCE_INLINE constexpr size_t ARRAY_SIZE(T) noexcept
{
    return 0;
}
#else
...
{
    return static_cast< take_sign_t<int16_t, T> >(
        ((static_cast<uint16_t>(x) & 0xFF00u) >> 8u) |
        ((static_cast<uint16_t>(x) & 0x00FFu) << 8u)
    );
}
template <typename T>
